...
@php
    $classes =
        $active ?? false
            ? "cursor-pointer rounded-lg bg-zinc-100"
            : "group cursor-pointer rounded-lg transition-all duration-300 hover:bg-zinc-100";
@endphp

<div {{ $attributes->merge(["class" => $classes]) }}>
    <a
        {{ $attributes }}
        class="group flex w-full items-center gap-5 p-2 text-zinc-700 transition-all duration-300"
