<x-app-layout>
    <div class="space-y-10">
        <div>
            <h1 class="flex gap-2 items-center"><i class="fi fi-rr-house-laptop"></i><span>Behaviours</span></h1>
        </div>
                    ...
                    @foreach($behaviours as $behaviour)
                        <tr class="bg-white border-b">
                            ...
                            <th class="px-6 py-4 font-normal text-zinc-700 whitespace-nowrap flex gap-4">
                                <a href="{{ route('behaviours.edit',$behaviour->id) }}" class="text-sky-500 hover:text-sky-700 transition-all duration-300">Edit</a>
                                <a href="#" data-delete-id="{{ $behaviour->id }}" data-modal-target="confirm-delete-modal" data-modal-toggle="confirm-delete-modal" class="delete-btn text-rose-500 hover:text-rose-700 transition-all duration-300">Delete</a>
                            </th>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-4 md:p-5 text-center space-y-4">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-zinc-500">Are you sure you want to delete this behaviour?</h3>

                    <form id="delete-form" action="{{ route('behaviours.index') }}/" method="POST" class="hidden">@csrf @method('DELETE')</form>
