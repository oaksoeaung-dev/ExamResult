                ...
                <div class="grid grid-cols-5 gap-3">
                    @foreach ($subjects as $subject)
                        @if ($class->subjects->find($subject->id))
                            <x-select-box name="subjects[]" :checked="true" value="{{ $subject->id }}">
                                {{ $subject->name }}
                            </x-select-box>
                        @else
                            <x-select-box name="subjects[]" :checked="false" value="{{ $subject->id }}">
                                {{ $subject->name }}
                            </x-select-box>
                        @endif
                    @endforeach
                ...
                <div class="grid grid-cols-5 gap-3">
                    @foreach ($activities as $activity)
                        @if ($class->activities->find($activity->id))
                            <x-select-box name="activities[]" :checked="true" value="{{ $activity->id }}">
                                {{ $activity->name }}
                            </x-select-box>
                        @else
                            <x-select-box name="activities[]" :checked="false" value="{{ $activity->id }}">
                                {{ $activity->name }}
                            </x-select-box>
                        @endif
                    @endforeach
                ...
                <div class="grid grid-cols-5 gap-3">
                    @foreach ($behaviours as $behaviour)
                        @if ($class->behaviours->find($behaviour->id))
                            <x-select-box name="behaviours[]" :checked="true" value="{{ $behaviour->id }}">
                                {{ $behaviour->name }}
                            </x-select-box>
                        @else
                            <x-select-box name="behaviours[]" :checked="false" value="{{ $behaviour->id }}">
                                {{ $behaviour->name }}
                            </x-select-box>
                        @endif
                    @endforeach
