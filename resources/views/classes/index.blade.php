                    ...
                    @foreach ($classes as $class)
                        <tr class="border-b bg-white">
                            ...
                            <th class="flex gap-4 whitespace-nowrap px-6 py-4 font-normal text-zinc-700">
                                <a href="{{ route("classes.show", $class->id) }}" class="text-teal-500 transition-all duration-300 hover:text-teal-700">View</a>
                                <a href="{{ route("classes.edit", $class->id) }}" class="text-sky-500 transition-all duration-300 hover:text-sky-700">Edit</a>
                                <a href="#" data-delete-id="{{ $class->id }}" data-modal-target="confirm-delete-modal" data-modal-toggle="confirm-delete-modal" class="delete-btn text-rose-500 transition-all duration-300 hover:text-rose-700">Delete</a>
                            </th>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="relative max-h-full w-full max-w-md p-4">
            <div class="relative rounded-lg bg-white shadow">
                ...
                <div class="space-y-4 p-4 text-center md:p-5">
                    ...
                    <form id="delete-form" action="{{ route("classes.index") }}/" method="POST" class="hidden">
                        @csrf
                        @method("DELETE")
                    </form>
